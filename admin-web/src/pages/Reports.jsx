...
import { Search, Plus, X, Upload, Trash2, Loader2 } from 'lucide-react';
...
export default function Reports() {
  ...
  const filtered = reportsData.filter(r => {
    ...
    const matchSearch = r.title.toLowerCase().includes(q) || r.report_number.toLowerCase().includes(q) || r.location.toLowerCase().includes(q);
    const matchStatus = filterStatus === "Semua" || r.status.toLowerCase() === filterStatus.toLowerCase().replace(" ", "_");
    const matchPriority = filterPriority === "Semua" || r.priority.toLowerCase() === filterPriority.toLowerCase();
  ...
  const handleAddReport = async (e) => {
    e.preventDefault();
    const priority = e.target.priority.value.toLowerCase();
    
    // Convert status to backend format if necessary 
    // Backend creates new reports as "menunggu" automatically.
    ...
    try {
      await api.post('/reports', newReportPayload);
  ...
  const handleUpdateStatus = async () => {
    ...
    const STATUS_FLOW = {
      ...
      "selesai":     "menunggu",
      "eskalasi":    "dalam_proses",
    };
    const nextStatus = STATUS_FLOW[selectedReport.status] || "menunggu";

    try {
      const res = await api.post(`/reports/${selectedReport.id}/status`, { status: nextStatus, description: `Status manual diupdate ke ${nextStatus.replace('_', ' ')}` });
      setSelectedReport(res.data);
      fetchReports(); // Refresh data table
    } catch(err) {
  ...
  const handleDeleteReport = async () => {
    if(!selectedReport) return;
    if(!window.confirm(`Yakin ingin menghapus laporan ${selectedReport.report_number}?`)) return;
  ...
  const openModal = (r) => { setSelectedReport(r); setActiveTab('Detail'); };
    ...
    <div className="p-6 md:p-7 flex flex-col gap-5 relative">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[900px]">
          ...
          <tbody>
            {isLoading ? <tr><td colSpan="9" className="text-center py-10"><Loader2 className="w-6 h-6 animate-spin mx-auto text-brand-primary" /></td></tr> : sortedAndFiltered.map(r => {
              ...
              return (
                <tr key={r.id} className="table-row border-b border-dark-border/40 last:border-0 cursor-pointer hover:bg-dark-hover transition-colors">
                  ...
                  <td className="py-3.5 px-3.5">
                    <button className="btn btn-ghost py-1.5 px-2.5 text-[11px]" onClick={() => openModal(r)}>Detail</button>
                  </td>
                </tr>
              );
            })}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-dark-bg/80 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedReport(null)}>
          <div className="bg-dark-card border border-dark-border rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 flex justify-between items-start bg-dark-hover">
              <div>
                <div className="font-mono text-[12px] text-brand-primary font-bold">{selectedReport.report_number}</div>
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                ...
                <div className="flex flex-col gap-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      ["Pelapor", selectedReport.reporter?.name || "-"],
                      ["Tanggal", new Date(selectedReport.created_at).toLocaleString('id-ID')],
                      ["Prioritas", selectedReport.priority],
                      ["Status", selectedReport.status],
                      ["Teknisi", (selectedReport.active_technicians || []).map(t => t.name).join(', ') || "Belum ditugaskan"],
                      ["Deadline SLA", new Date(selectedReport.sla_deadline).toLocaleString('id-ID')],
                    ].map(([k,v]) => (
              ...
              {activeTab === 'Riwayat' && (
                <div className="flex flex-col gap-0 animate-fade-in pt-2 pl-4">
                  {(selectedReport.histories || []).map((h, i) => (
                    <div key={i} className="relative pb-6 pl-6 border-l-2 border-dark-border last:border-l-transparent last:pb-0">
                      ...
                      <div className="text-[12px] text-ui-muted">{new Date(h.created_at).toLocaleString('id-ID')} {h.description ? `· ${h.description}` : ''} {(h.user && h.user.name) ? `oleh ${h.user.name}` : ''}</div>
                    </div>
                  ))}
                  {(selectedReport.histories || []).length === 0 && <div className="text-[12px] text-ui-muted py-4">Belum ada riwayat tercatat.</div>}
                </div>
              )}
            </div>
            ...
            <div className="px-6 py-4 bg-dark-hover flex gap-3 border-t border-dark-border">
              <button className="btn btn-primary flex-1 hover:brightness-110" onClick={handleUpdateStatus}>
                 Update Status <span className="font-normal opacity-70 ml-1">→ {( { "menunggu": "Dalam Proses", "dalam_proses": "Selesai", "selesai": "Menunggu", "eskalasi": "Dalam Proses" }[selectedReport.status] || "Menunggu" )}</span>
              </button>
              <button className="btn btn-danger p-2 px-3" onClick={handleDeleteReport} title="Hapus Laporan"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleAddReport}>
              <div className="p-6 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <textarea name="description" className="input text-[12px] h-20 py-2 resize-none" placeholder="Jelaskan detail masalah..."></textarea>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="category" className="input text-[12px]" required>
                      {["Listrik", "HVAC", "Plumbing", "Lab", "Jaringan", "Lift", "Bangunan", "Umum"].map(c => (
                        <option key={c} value={c} className="bg-dark-bg">{c}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    {/* Dropdown priority 4 level */}
                    ...
                    <select name="priority" className="input text-[12px]" required>
                      <option value="Rendah"  className="bg-dark-bg">🟢 Rendah   — tidak mendesak</option>
                      <option value="Sedang"  className="bg-dark-bg">🔵 Sedang   — perlu ditangani</option>
                      <option value="Tinggi"  className="bg-dark-bg">🟡 Tinggi   — ganggu operasional</option>
                      <option value="Kritis"  className="bg-dark-bg">🔴 Kritis   — ancaman keselamatan</option>
                    </select>
                  </div>
                </div>
                <div>
